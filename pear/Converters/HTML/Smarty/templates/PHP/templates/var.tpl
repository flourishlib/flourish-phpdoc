...
{if !$vars[vars].static}
{if $show == 'summary'}
	<p>var {$vars[vars].var_name}, {$vars[vars].sdesc}</p>
{else}
	...
	<!-- start {if $access == 'protected' || $vars[vars].internal}protected {/if}variable -->
	<h3 id="{$vars[vars].var_dest}" class="{if $access == 'protected' || $vars[vars].internal}protected {/if}variable"><span class="resolution">-></span><span class="name">{$vars[vars].var_name|replace:"\$":""}</span> <span class="visibility">{if $vars[vars].internal}internal {/if}{$access}</span>{*<a href="#top" class="top">Top</a>*}</h3>
	
	<div class="{if $access == 'protected' || $vars[vars].internal}protected {/if}variable">	
